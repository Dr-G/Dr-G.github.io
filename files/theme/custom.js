jQuery(function($) {
  ...
  var Theme = {
    ...
    checkCart: function() {
      return $("#panel-one .wsite-nav-cart").length;
    },
    moveCart: function() {
      if ($("#wsite-nav-cart-num").text() != "-") {
        var cart = $('#navhidden').detach();
        $("#logo").after(cart);
        $("#wsite-nav-cart-num").attr("href", $("#wsite-com-minicart-checkout-button").attr("href"));
      }
    },
    checkLogin: function() {
  ...
  $(document).ready(function() {
    ...
    Theme.swipeInit();
    Theme.hideCart();
    Theme.interval(Theme.checkCart, Theme.moveCart, 500, 8);
    Theme.interval(Theme.checkLogin, Theme.moveLogin, 500, 8);
    Theme.footerCheck();
  });
